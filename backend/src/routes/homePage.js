...
const dateDifferential = require('../utility/utilityFunctions')
    .getDateDifferential;
...
router.get('/:username/suggested', async (req, res, next) => {
    try {
        ...
        return_obj.profile_picture = convertBuffer2Base64(
            user,
            'profile_picture',
        );
        ...
        return res
            .status(200)
            .json({ profile_info: return_obj, suggested_users_to_follow: [] });
    } catch (err) {
        return next(err);
    }
});
...
router.get('/:username/:slicepostsreq', async (req, res, next) => {
    try {
        ...
        const user_logged_in = await User.findOne(
            { username: req.params.username },
            returned_fields,
        )
            .populate({
        ...
        for (const post of all_posts_following_self_sorted) {
            ...
            photo_obj.date_posted = dateDifferential(
                post.created_at,
                new Date(Date.now()),
            );
