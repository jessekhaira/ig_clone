...
describe('GET /', () => {
    ...
    afterEach(async (done) => {
        const results = await request(app).post('/accounts/login').send({
    ...
    test('User 19 seeded to follow multiple other users -- homepage should return appropriate number of posts', async (done) => {
        const results_firstpage = await request(app)
            .get('/homepage/testUser19/1')
            .set('Authorization', `${token}`)
            .expect(200)
            .expect('Content-Type', /json/);

        const results_secondpage = await request(app)
            .get('/homepage/testUser19/2')
            .set('Authorization', `${token}`)
            .expect(200)
            .expect('Content-Type', /json/);

        const results_thirdpage = await request(app)
