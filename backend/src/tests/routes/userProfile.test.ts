...
describe('Grouping tests that test PUT endpoints built off /:userprofile route', () => {
    ...
    test("Testing PUT request to follow a given user", async (done) => {
        ...
        expect(testUser2.followers).toContainEqual(testing2._id);
        expect(returnedData).toHaveProperty('Success');
        done();
    });

    test("Testing PUT request to unfollow a given user", async (done) => {
        const returnedData = (await request(app)
            .put(`/testUser5/follow/testUser2`)
            .set('Authorization', accessToken)
            .expect(200)
            .expect('Content-Type', /json/)
        ).body;

        const testUser5 = await User.findOne({username: 'testUser5'});
        const testUser2 = await User.findOne({username: 'testUser2'});
        expect(testUser5.following).not.toContainEqual(testUser2._id);
        expect(testUser2.followers).not.toContainEqual(testUser5._id);
        expect(returnedData).toHaveProperty('Success');
        done();
    });  
    
    test("Testing PUT request to unfollow then follow a given user", async (done) => {
        await request(app)
        .put(`/testUser8/follow/testUser2`)
        .set('Authorization', accessToken)
        .expect(200)
        .expect('Content-Type', /json/);
        ...
        const testUser5 = await User.findOne({username: 'testUser5'});
        const testUser2 = await User.findOne({username: 'testUser2'});
        expect(testUser5.following).toContainEqual(testUser2._id);
...
describe('Grouping tests that test DELETE endpoints built off /:userprofile route', () => {
    ...
    test("Testing DELETE request to delete new photo post, should succeed", async (done) => {
        ...
        testUser2.photos.forEach(photo => {
            expect(photo._id).not.toEqual(grid_img_id);
        })
        done();
    });
})
